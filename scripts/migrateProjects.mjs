// API를 통한 프로젝트 마이그레이션 스크립트
// 로컬 서버가 실행 중이어야 합니다: npm run dev
...
const projects = [
  {
    ...
    image: '/images/common/img_user.png',
...
function createFormData(project) {
  ...
  // detail 필드는 전체를 JSON 문자열로 변환
  if (project.detail) {
    formData.append('detail', JSON.stringify(project.detail));
  }
...
async function migrateProjects() {
  ...
  try {
    for (let i = 0; i < projects.length; i++) {
      ...
      console.log(`\n[${i + 1}/${projects.length}] "${project.title}" 처리 중...`);
      ...
      try {
        ...
        console.log(`✅ 등록 완료 (ID: ${result.id})`);
      } catch (error) {
        console.error(`❌ "${project.title}" 등록 실패:`, error.message);
        throw error;
      }
    }

    console.log('\n🎉 모든 프로젝트 마이그레이션 완료!');
    console.log(`총 ${projects.length}개 프로젝트가 등록되었습니다.`);
  } catch (error) {
    console.error('\n❌ 마이그레이션 실패');
    process.exit(1);
  }
